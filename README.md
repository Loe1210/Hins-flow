...
Hins-flow v2 是一次完整的流程重构，重点是让它更可靠、更全能，同时减少不必要的步骤：

- 全局只安装一个 `hins-flow`，日常始终通过 `/hins-flow` 进入，不再要求用户分别安装或调用一组 Matt skills；
- 将 22 个稳定的 Matt Pocock 工程方法固定并适配为内部能力，安装器会一起校验，因此换到另一台电脑也不需要再补装这些依赖；
- 根据任务的规模、风险和不确定性自动选择 Quick、Standard、Large 或 Wayfinding 路线，小改动不会被迫走完整长流程；
- 合并重复的需求追问、测试和评审，只有相关代码、规格、配置或环境变化时才使已有证据失效；
- 用户需要阅读的需求、计划、Gate、评审、交付说明和下一步提示默认使用简体中文；
- 安装和升级采用完整校验、原子替换、自动备份与失败回滚，旧版独立 skills 不会被擅自删除。

从 v1 升级不需要更换命令，重新执行下面的一条安装命令即可。升级后完全退出并重新打开 Codex，再新建任务使用 `/hins-flow`。
...
```text
/hins-flow 我想给这个项目增加一个搜索功能
```

也可以给出更完整的目标：

```text
/hins-flow 增加项目内搜索，支持关键词筛选和分页，保持现有行为兼容，并补充空结果、权限和异常场景测试
```
